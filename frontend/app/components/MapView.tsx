...
function ViewportController({
  ...
  useEffect(() => {
    ...
    async function fit() {
      ...
      dbgSet(
        `debug:bounds OK sw=${sw.lat.toFixed(5)},${sw.lng.toFixed(5)} ne=${ne.lat.toFixed(
          5,
        )},${ne.lng.toFixed(5)}`,
      );
    ...
    <div className="w-full h-[600px] rounded-lg overflow-hidden border relative">
      ...
      <div className="absolute top-3 left-3 z-[1000] bg-white/90 border rounded px-3 py-2 text-sm space-y-1">
        {hasData ? (
          <>
            <div>
              {pins.length} target{pins.length > 1 ? 's' : ''} affiché{pins.length > 1 ? 's' : ''}{' '}
              (max {maxPins})
            </div>
      ...
      <MapContainer center={fallbackCenter} zoom={13} style={{ height: '100%', width: '100%' }}>
        ...
        {tourLatLngs ? (
          <Polyline positions={tourLatLngs} pathOptions={{ weight: 4, opacity: 0.9 }} />
        ) : null}
